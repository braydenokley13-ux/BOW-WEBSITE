...
async function cohortStudentIds(cohortId: string): Promise<string[]> {
...
export async function getCohortAnalytics(cohortId: string): Promise<CohortAnalytics> {
  const cohort = (await getDb().prepare("SELECT name FROM cohorts WHERE id = ?").get(cohortId)) as any;
  const modules = (await getSelfModules());
  const ids = (await cohortStudentIds(cohortId));
  const n = ids.length;
  ...
  for (const id of ids) {
    const score = (await getStudentScore(id, 0));
    if (score) {
      scoreSum += score.bowScore;
      topScore = Math.max(topScore, score.bowScore);
    }
    const stats = (await computeStats(id, 0));
    ...
    const views = (await getSelfModuleViews(id));
    ...
    const sections = (await getQuizModuleSections(id));
