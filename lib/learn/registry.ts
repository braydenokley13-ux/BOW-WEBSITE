...
export const BLOCK_REGISTRY: Record<BlockType, BlockRegistryEntry> = {
  ...
  rank: {
    ...
    label: "Rank",
    category: "decision",
    schema: RankBlockSchema,
    defaults: { type: "rank", prompt: "", items: [], correctOrder: [], grading: "weighted", effects: [], bands: [], points: 1 } as Record<string, unknown>,
    summarize: (b) => `Rank: ${((b as { items?: unknown[] }).items ?? []).length} item(s)`,
    capabilities: DECISION_CAPS,
  },
  categorize: {
    type: "categorize",
    ...
    label: "Categorize",
    category: "decision",
    schema: CategorizeBlockSchema,
    defaults: { type: "categorize", prompt: "", categories: [], items: [], grading: "weighted", effects: [], bands: [], points: 1 } as Record<string, unknown>,
    summarize: (b) => `Categorize: ${((b as { items?: unknown[] }).items ?? []).length} item(s), ${((b as { categories?: unknown[] }).categories ?? []).length} bucket(s)`,
    capabilities: DECISION_CAPS,
  },
  drag_drop: {
    type: "drag_drop",
    ...
    label: "Drag & Drop",
    category: "decision",
    schema: DragDropBlockSchema,
    defaults: { type: "drag_drop", prompt: "", categories: [], items: [], grading: "weighted", effects: [], bands: [], points: 1 } as Record<string, unknown>,
    summarize: (b) => `Drag & drop: ${((b as { items?: unknown[] }).items ?? []).length} item(s), ${((b as { categories?: unknown[] }).categories ?? []).length} bucket(s)`,
    capabilities: DECISION_CAPS,
  },
  match: {
    type: "match",
    ...
    label: "Match",
    category: "decision",
    schema: MatchBlockSchema,
    defaults: { type: "match", prompt: "", pairs: [], grading: "weighted", effects: [], bands: [], points: 1 } as Record<string, unknown>,
    summarize: (b) => `Match: ${((b as { pairs?: unknown[] }).pairs ?? []).length} pair(s)`,
    capabilities: DECISION_CAPS,
  },
  tradeoff_matrix: {
    ...
    category: "decision",
    schema: TradeoffMatrixBlockSchema,
    defaults: { type: "tradeoff_matrix", prompt: "", criteria: [], options: [], grading: "weighted", effects: [], bands: [], points: 0 } as Record<string, unknown>,
    summarize: (b) => `Tradeoff matrix: ${((b as { options?: unknown[] }).options ?? []).length} option(s) x ${((b as { criteria?: unknown[] }).criteria ?? []).length} criteria`,
    capabilities: DECISION_CAPS,
  },
  forecast: {
    type: "forecast",
    ...
    label: "Forecast",
    category: "decision",
    schema: ForecastBlockSchema,
    defaults: { type: "forecast", prompt: "", unit: "number", correctValue: 0, tolerance: 1, grading: "weighted", effects: [], bands: [], points: 1 } as Record<string, unknown>,
    summarize: (b) => `Forecast: target ${(b as { correctValue?: number }).correctValue ?? 0} +/- ${(b as { tolerance?: number }).tolerance ?? 0}`,
    capabilities: DECISION_CAPS,
  },
  table: {
    type: "table",
    ...
    defaults: { type: "table", columns: [], rows: [] } as Record<string, unknown>,
    summarize: (b) => `Table: ${((b as { rows?: unknown[] }).rows ?? []).length} row(s)`,
    capabilities: CONTENT_CAPS,
  },
  chart: {
    type: "chart",
    ...
    defaults: { type: "chart", chartKind: "bar", unit: "number", series: [] } as Record<string, unknown>,
    summarize: (b) => `Chart (${(b as { chartKind?: string }).chartKind ?? "bar"}): ${((b as { series?: unknown[] }).series ?? []).length} point(s)`,
    capabilities: CONTENT_CAPS,
  },
  timeline: {
    type: "timeline",
    ...
    defaults: { type: "timeline", events: [] } as Record<string, unknown>,
    summarize: (b) => `Timeline: ${((b as { events?: unknown[] }).events ?? []).length} event(s)`,
    capabilities: CONTENT_CAPS,
  },
};
