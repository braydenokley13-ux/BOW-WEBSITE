...
function toRow(r: BaseRow, position: number, withBadges: boolean): LeaderboardRow {
  return {
    studentId: r.id,
    position,
    ...
    xp: r.xp,
    badgeCount: withBadges ? countStudentBadges(r.id) : 0,
    topBadgeIcon: withBadges ? (getMostRecentBadge(r.id)?.icon ?? null) : null,
  };
}
