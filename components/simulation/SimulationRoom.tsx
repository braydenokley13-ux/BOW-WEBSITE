...
import { startSimulation, makeSimDecision } from "@/app/actions/simulation";
...
export default function SimulationRoom({ firstName, turns, initialState }: Props) {
  ...
  const finalReport: SimReport | null = report ?? (state?.completed ? gradeSimulation(state.decisions) : null);
  ...
  const onChoose = async (choiceId: string) => {
    if (busy) return;
    setBusy(true);
    const res = await makeSimDecision(choiceId);
    setBusy(false);
    if (res.ok && res.state) {
      setState(res.state);
...
function Entry({ firstName, busy, onStart }: { firstName: string; busy: boolean; onStart: () => void }) {
  const facts = [
    { label: "Your team", value: "Westbrook Wolves" },
    { label: "Starting cap", value: fmtCap(START_CAP) },
    { label: "Starting record", value: "0–0" },
    { label: "Turns", value: `${TOTAL_TURNS} decisions` },
  ];
  return (
    <>
      ...
      <p style={{ margin: "0 0 22px", fontFamily: "var(--font-interface)", fontSize: 16, lineHeight: 1.6, color: "#b9bcc4", maxWidth: 560 }}>
        Ten turns, {firstName}. Each one is a real front-office decision against a salary cap and a win-loss record. Every call teaches one economics idea — and at the end you get a BOW Economics Grade.
      </p>
      ...
      <button
        onClick={onStart}
        disabled={busy}
        style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 15, letterSpacing: "0.05em", textTransform: "uppercase", padding: "15px 30px", border: "none", background: busy ? "var(--bow-inactive)" : "var(--bow-orange)", color: "#fff", borderRadius: 4, cursor: busy ? "wait" : "pointer" }}
      >
...
function Hud({ state }: { state: SimState }) {
  const items = [
    ...
    { label: "Turn", value: `${Math.min(state.turn, TOTAL_TURNS)} / ${TOTAL_TURNS}`, accent: "#6f8bff" },
  ];
  return (
...
function Playing({ state, turns, busy, onChoose }: { state: SimState; turns: PublicTurn[]; busy: boolean; onChoose: (id: string) => void }) {
  ...
  return (
    <>
      ...
      <Hud state={state} />
      <div style={{ ...card, borderTop: "4px solid var(--bow-orange)", padding: "clamp(18px,2.6vw,26px)" }}>
    ...
    <>
      ...
      <div style={{ ...card, borderTop: "4px solid var(--bow-positive)", padding: "clamp(18px,2.6vw,26px)" }}>
        ...
        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid var(--bow-dark-border)" }}>
          <span style={{ fontFamily: "var(--font-data)", fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--bow-orange)" }}>
...
function ReportCard({ report, onPlayAgain, busy }: { report: SimReport; onPlayAgain: () => void; busy: boolean }) {
  const gradeColor = report.grade === "A" ? "#5fcf99" : report.grade === "B" ? "#6f8bff" : report.grade === "C" ? "var(--bow-warning)" : "var(--bow-orange)";
  const soundCount = report.conceptsApplied.filter((c) => c.sound).length;
  return (
    <>
      <span style={{ fontFamily: "var(--font-data)", fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--bow-orange)" }}>
        Final Report · Westbrook Wolves
      </span>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "stretch", marginBottom: 22 }}>
        <div style={{ ...card, borderTop: `4px solid ${gradeColor}`, padding: "20px 26px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 140 }}>
          ...
          <div style={{ fontFamily: "var(--font-data)", fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: "#9a9da6" }}>Econ Grade · {report.gmScore}/100</div>
        </div>
        <div style={{ flex: 1, minWidth: 200, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 10 }}>
          ...
          <Stat label="Wins gained" value={String(report.totalWins)} />
          <Stat label="Sound calls" value={`${soundCount} / ${report.conceptsApplied.length}`} />
        </div>
      </div>

      <div style={{ ...card, padding: "clamp(18px,2.4vw,24px)", marginBottom: 18 }}>
        <span style={{ fontFamily: "var(--font-data)", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--bow-orange)" }}>
          Your style: {report.pattern.name}
        </span>
      ...
      <span style={{ fontFamily: "var(--font-data)", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#9a9da6", display: "block", marginBottom: 10 }}>
        Concepts you applied
      </span>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
        {report.conceptsApplied.map((c, i) => (
          <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "var(--font-data)", fontSize: 11.5, letterSpacing: "0.03em", color: c.sound ? "#5fcf99" : "#9a9da6", border: `1px solid ${c.sound ? "var(--bow-positive)" : "var(--bow-dark-border)"}`, borderRadius: 999, padding: "6px 12px" }}>
            {c.sound ? "✓" : "•"} {c.concept}
          </span>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <button
          onClick={onPlayAgain}
          disabled={busy}
          style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 14, letterSpacing: "0.05em", textTransform: "uppercase", padding: "13px 26px", border: "none", background: busy ? "var(--bow-inactive)" : "var(--bow-orange)", color: "#fff", borderRadius: 4, cursor: busy ? "wait" : "pointer" }}
        >
