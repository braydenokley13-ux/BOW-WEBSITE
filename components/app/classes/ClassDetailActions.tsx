  ...
  return (
    <div className="ops-panel">
      <div className="ops-section-head">
        <h2 className="ops-section-title">Actions</h2>
      </div>
      ...
      {full && <p className="ops-error" style={{ marginTop: 8 }}>Class is at capacity.</p>}
      {error && !modal && <p role="alert" className="ops-error" style={{ marginTop: 8 }}>{error}</p>}

      <Modal open={modal === "status"} onClose={() => setModal(null)} title={`Move Class to ${statusTarget.replace(/_/g, " ")}`} dismissible={!busy}>
        <div className="ops-field">
          <label htmlFor="class-status-reason">Operational reason</label>
          <textarea id="class-status-reason" rows={4} value={statusReason} onChange={(event) => setStatusReason(event.target.value)} placeholder="What happened, and what should the next operator know?" />
        </div>
        {error && <p role="alert" className="ops-error" style={{ margin: "10px 0" }}>{error}</p>}
      ...
      <Modal open={modal === "assign"} onClose={() => setModal(null)} title="Assign Instructor" dismissible={!busy}>
        <div className="ops-field">
          ...
          <select id="class-instructor" value={pickedInstructor} onChange={(e) => setPickedInstructor(e.target.value)}>
        ...
        <div className="ops-field">
          <label htmlFor="class-instructor-role">Assignment role</label>
          <select id="class-instructor-role" value={pickedRole} onChange={(e) => setPickedRole(e.target.value as "lead" | "additional")}>
      ...
      <Modal open={modal === "session"} onClose={() => setModal(null)} title="Schedule Session" dismissible={!busy}>
        <div className="ops-field">
          <label htmlFor="class-session-date">Session date and time</label>
          <input id="class-session-date" type="datetime-local" value={sessionDate} onChange={(e) => setSessionDate(e.target.value)} />
        </div>
        <div className="ops-field">
          <label htmlFor="class-session-timezone">Delivery timezone</label>
          <input
        ...
        <div className="ops-field">
          <label htmlFor="class-session-location">Location (optional)</label>
          <input id="class-session-location" placeholder="Room, field, or meeting link" value={sessionLocation} onChange={(e) => setSessionLocation(e.target.value)} />
        </div>
        {error && <p role="alert" className="ops-error">{error}</p>}
        <div className="ops-form-footer">
          <Button
      ...
      <Modal open={modal === "enroll"} onClose={() => setModal(null)} title="Enroll Student" dismissible={!busy}>
        <div className="ops-field">
          <label htmlFor="class-enroll-student">Student</label>
          <select id="class-enroll-student" value={pickedStudent} onChange={(e) => setPickedStudent(e.target.value)}>
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Remove Instructor" dismissible={!busy}>
        <div className="ops-field">
          <label htmlFor={`remove-instructor-reason-${instructorId}`}>Staffing decision reason</label>
          <textarea id={`remove-instructor-reason-${instructorId}`} rows={4} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Why is this change necessary, and who owns coverage?" />
        </div>
        {error && <p role="alert" className="ops-error">{error}</p>}
        <div className="ops-form-footer">
          <Button
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Withdraw Student" dismissible={!busy}>
        <div className="ops-field">
          <label htmlFor={`withdraw-reason-${studentId}`}>Withdrawal reason</label>
          <textarea id={`withdraw-reason-${studentId}`} rows={4} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Record the reason for roster history." />
        </div>
        {error && <p role="alert" className="ops-error">{error}</p>}
        <div className="ops-form-footer">
          <Button
