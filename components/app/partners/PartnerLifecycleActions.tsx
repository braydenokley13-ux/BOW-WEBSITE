...
export default function PartnerLifecycleActions({
  ...
  return (
    <section aria-labelledby="partner-lifecycle-title" style={{ background: "var(--bow-white)", border: "1px solid var(--border-rule)", borderRadius: 6, padding: 22 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        <div>
          <span style={{ fontFamily: "var(--font-data)", fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--bow-slate)" }}>Organization lifecycle</span>
          <h2 id="partner-lifecycle-title" style={{ margin: "5px 0 0", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 20, textTransform: "uppercase", color: "var(--bow-ink)" }}>
            {organizationName}
          </h2>
        </div>
      ...
      <p style={{ margin: "12px 0 0", fontFamily: "var(--font-interface)", fontSize: 14, lineHeight: 1.55, color: "var(--bow-slate)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 8, marginTop: 14 }}>
        {[
          ...
          <div key={String(label)} style={{ padding: "10px 11px", border: "1px solid var(--border-rule)", borderRadius: 4, minWidth: 0 }}>
            <span style={{ display: "block", fontFamily: "var(--font-data)", fontSize: 9, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--bow-slate)" }}>{label}</span>
            <strong style={{ display: "block", marginTop: 3, fontFamily: "var(--font-display)", fontSize: 20, color: "var(--bow-ink)" }}>{value}</strong>
          </div>
        ))}
      </div>
      {latestLifecycleEvent && (
        <div style={{ marginTop: 14, padding: "11px 12px", background: "var(--bow-paper)", borderLeft: "3px solid var(--bow-blue)" }}>
          <span style={{ fontFamily: "var(--font-data)", fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--bow-slate)" }}>Latest lifecycle reason</span>
          <p style={{ margin: "4px 0 0", fontFamily: "var(--font-interface)", fontSize: 13, lineHeight: 1.5, color: "var(--bow-ink)" }}>{latestLifecycleEvent}</p>
        </div>
      )}
      ...
      {notice && (
        <p
          ...
          style={{ margin: "14px 0 0", color: notice.tone === "negative" ? "var(--bow-negative)" : "var(--bow-positive)", fontFamily: "var(--font-interface)", fontSize: 13 }}
        >
          {notice.message}
        </p>
      )}
      ...
      {protectedOrganization ? (
        <p style={{ margin: "16px 0 0", fontFamily: "var(--font-interface)", fontSize: 14, color: "var(--bow-slate)" }}>
          ...
          {options.map((option) => {
            ...
            const borderColor = option.tone === "negative"
              ...
              : option.tone === "warning"
                ...
                : "var(--bow-positive)";
            return (
              <div key={option.nextStatus} style={{ border: "1px solid var(--border-rule)", borderTop: `3px solid ${borderColor}`, borderRadius: 5, padding: 15 }}>
                <h3 style={{ margin: 0, fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 15, textTransform: "uppercase", color: "var(--bow-ink)" }}>{option.label}</h3>
                <p style={{ margin: "6px 0 12px", fontFamily: "var(--font-interface)", fontSize: 13, lineHeight: 1.5, color: "var(--bow-slate)" }}>{option.description}</p>
                {blocked ? (
                  <p role="status" style={{ margin: 0, fontFamily: "var(--font-interface)", fontSize: 13, lineHeight: 1.5, color: "var(--bow-warning-text)" }}>
                    Close is blocked by {closeBlockerSummary}. Finish or retire that operating work first.
                  </p>
                ) : (
                  <form onSubmit={(event) => void submit(option, event)}>
                    <label htmlFor={`partner-lifecycle-${option.nextStatus}`} style={{ display: "block", marginBottom: 5, fontFamily: "var(--font-data)", fontSize: 10, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--bow-slate)" }}>
                      {option.reasonLabel}
                    </label>
                    <textarea
                      ...
                      style={{ width: "100%", resize: "vertical", border: "1px solid var(--border-rule)", borderRadius: 4, padding: "9px 10px", fontFamily: "var(--font-interface)", fontSize: 13, color: "var(--bow-ink)", background: "var(--bow-white)" }}
                    />
                    {option.nextStatus === "closed" && (
                      <label style={{ display: "flex", alignItems: "flex-start", gap: 8, marginTop: 9, fontFamily: "var(--font-interface)", fontSize: 12, lineHeight: 1.45, color: "var(--bow-ink)" }}>
                    ...
                    <button
                      type="submit"
                      disabled={pendingStatus !== null}
                      style={{ marginTop: 9, border: `1px solid ${borderColor}`, borderRadius: 4, background: option.tone === "positive" ? "var(--bow-positive-solid)" : "transparent", color: option.tone === "positive" ? "var(--bow-white)" : option.tone === "warning" ? "var(--bow-warning-text)" : "var(--bow-ink)", padding: "8px 12px", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 11, letterSpacing: "0.05em", textTransform: "uppercase", cursor: pendingStatus ? "wait" : "pointer", opacity: pendingStatus ? 0.65 : 1 }}
                    >
