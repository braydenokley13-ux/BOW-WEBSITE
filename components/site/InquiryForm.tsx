...
/* ============================================================
 * Get Involved inquiry funnel — a clean, client-side
 * reimplementation of the prototype's multi-step inquiry flow
 * (state keys inquiryPath / inquiryStep / inquiryData and the
 * setInquiryPath / setInquiryField / advanceInquiry / submitInquiry
 * handlers). Front-end only — no backend yet.
 * ============================================================ */

type Step = 1 | 2 | 3;
type Status = "idle" | "editing" | "unavailable";
...
export default function InquiryForm({ initialPathId }: { initialPathId?: string }) {
  ...
  const validate = (): FormErrors => {
    ...
    path.fields.forEach((f) => {
      if (f.required) {
        ...
        else if (f.type === "email" && !/.+@.+\..+/.test(val)) errs[f.name] = "Enter a valid email address.";
      }
    });
    return errs;
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      {step === 2 && path && (
        <div>
          <button onClick={goBack} style={{ ...ghostButtonStyle, fontFamily: "var(--font-data)", fontWeight: 400, fontSize: 12, letterSpacing: "0.08em", marginBottom: 24, padding: 0, display: "flex", alignItems: "center", gap: 8 }}>← Back</button>
          ...
          <div style={{ background: "var(--bow-dark-surface)", border: "1px solid var(--bow-dark-border)", borderTop: "4px solid var(--bow-blue)", padding: "clamp(22px,3vw,36px)" }}>
              ...
              {path.fields.map((field: InquiryField) => {
                ...
                return (
                  <div key={field.name} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    {field.type === "text" && (
                      <input id={`inq-${field.name}`} value={value} onChange={(e) => setField(field.name, e.target.value)} type="text" placeholder={field.placeholder} aria-required={field.required} style={inputStyle} />
                    )}
                    {field.type === "email" && (
                      <input id={`inq-${field.name}`} value={value} onChange={(e) => setField(field.name, e.target.value)} type="email" placeholder={field.placeholder} aria-required={field.required} style={inputStyle} />
                    )}
                    {field.type === "select" && (
                      <select id={`inq-${field.name}`} value={value} onChange={(e) => setField(field.name, e.target.value)} aria-required={field.required} style={inputStyle}>
                    ...
                    {field.type === "textarea" && (
                      <textarea id={`inq-${field.name}`} value={value} onChange={(e) => setField(field.name, e.target.value)} rows={4} placeholder={field.placeholder} aria-required={field.required} style={{ ...inputStyle, resize: "vertical" }} />
                    )}
                    {error && (
                      <span role="alert" style={{ fontFamily: "var(--font-data)", fontSize: 12, color: "var(--bow-negative)", letterSpacing: "0.02em" }}>{error}</span>
                    )}
                  </div>
                );
              })}
            ...
            <div style={{ marginTop: 28, display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
              ...
              <button onClick={goBack} style={ghostButtonStyle}>Back</button>
            </div>
          </div>
        </div>
      )}
      ...
      {step === 3 && path && (
        <div>
          <button onClick={goBack} style={{ ...ghostButtonStyle, fontFamily: "var(--font-data)", fontWeight: 400, fontSize: 12, letterSpacing: "0.08em", marginBottom: 24, padding: 0, display: "flex", alignItems: "center", gap: 8 }}>← Edit</button>
          ...
          <div style={{ background: "var(--bow-dark-surface)", border: "1px solid var(--bow-dark-border)", marginBottom: 24 }}>
            ...
            {reviewItems.map((item) => (
              <div key={item.label} style={{ display: "grid", gridTemplateColumns: "160px 1fr", gap: 12, padding: "14px 20px", borderBottom: "1px solid var(--bow-dark-border)", alignItems: "start" }}>
                ...
                <span style={{ fontFamily: "var(--font-interface)", fontSize: 15, color: "#fff", lineHeight: 1.4, wordBreak: "break-word" }}>{item.value}</span>
              </div>
            ))}
          </div>
          ...
          {status === "unavailable" ? (
            ...
            <>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
                <Button variant="primary" size="lg" onClick={submit}>Send Inquiry</Button>
                <button onClick={goBack} style={ghostButtonStyle}>Edit</button>
              </div>
