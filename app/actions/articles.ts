...
async function mirrorArticle(id: string): Promise<MirrorOutcome> {
  const row = (await articlesDb().prepare("SELECT * FROM articles WHERE id = ?").get(id));
...
async function uniqueSlug(db: any, title: string, excludeId: string | null): Promise<string> {
  const base = slugify(title) || "untitled";
  let candidate = base;
  let n = 2;
  const clash = db.prepare("SELECT id FROM articles WHERE slug = ? AND id != ?");
  while ((await clash.get(candidate, excludeId ?? ""))) {
    candidate = `${base}-${n++}`;
  }
  return candidate;
}
...
async function snapshotRevision(db: any, articleId: string) {
  const row = (await db.prepare("SELECT title, dek, body, category, tags FROM articles WHERE id = ?").get(articleId)) as any;
  if (!row) return;
  (await db.prepare(
...
export async function saveArticle(id: string | null, input: ArticleInput): Promise<SaveArticleResult> {
  ...
  if (id) {
    const existing = (await db.prepare("SELECT * FROM articles WHERE id = ?").get(id)) as any;
    if (!existing) return { ok: false, error: "Article not found." };
    (await snapshotRevision(db, id));
    // Drafts track their title; published slugs are frozen.
    const slug = existing.status === "published" ? existing.slug : (await uniqueSlug(db, v.title, id));
    (await db.prepare(
  ...
  const slug = (await uniqueSlug(db, v.title, null));
  (await db.prepare(
...
export async function submitPaper(input: PaperInput): Promise<SaveArticleResult> {
  ...
  const submissionLimit = (await consumeRateLimit("article-submission-user", me.id, {
      limit: 5,
  ...
  if ((await countOpenSubmissions(me.id)) >= MAX_OPEN_SUBMISSIONS) {
  ...
  const now = Date.now();
  const newId = `art-${randomUUID().slice(0, 8)}`;
  const slug = (await uniqueSlug(db, title, null));
  (await db.prepare(
...
export async function setArticleStatus(id: string, publish: boolean): Promise<ArticleActionResult> {
  ...
  if (publish) {
    ...
    (await db.prepare("UPDATE articles SET status = 'published', published_at = COALESCE(published_at, ?), updated_at = ? WHERE id = ?").run(
            Date.now(),
            Date.now(),
            id,
          ));
    ...
    (await db.prepare("UPDATE articles SET status = 'draft', updated_at = ? WHERE id = ?").run(updatedAt, id));
  }
  const mirror = await mirrorArticle(id);
...
export async function setArticleFeatured(id: string, featured: boolean): Promise<ArticleActionResult> {
  await requireRole("admin");
  const db = articlesDb();
  const existing = (await db.prepare("SELECT slug FROM articles WHERE id = ?").get(id)) as any;
  if (!existing) return { ok: false };
  (await db.prepare("UPDATE articles SET featured = ?, updated_at = ? WHERE id = ?").run(featured ? 1 : 0, Date.now(), id));
  const mirror = await mirrorArticle(id);
  refresh(existing.slug);
  return { ok: true, warning: mirror.warning };
}
...
export async function deleteArticle(id: string): Promise<ArticleActionResult> {
  ...
  (await db.prepare("DELETE FROM article_revisions WHERE article_id = ?").run(id));
...
export async function restoreRevision(articleId: string, revisionId: number): Promise<ArticleActionResult> {
  await requireRole("admin");
  const db = articlesDb();
  const rev = (await db.prepare("SELECT * FROM article_revisions WHERE id = ? AND article_id = ?").get(revisionId, articleId)) as any;
  const existing = (await db.prepare("SELECT slug, status FROM articles WHERE id = ?").get(articleId)) as any;
  if (!rev || !existing) return { ok: false };
  (await snapshotRevision(db, articleId));
  const slug = existing.status === "published" ? existing.slug : (await uniqueSlug(db, rev.title, articleId));
  (await db.prepare("UPDATE articles SET slug=?, title=?, dek=?, body=?, category=?, tags=?, updated_at=? WHERE id=?").run(
        slug,
        rev.title,
        rev.dek,
        rev.body,
