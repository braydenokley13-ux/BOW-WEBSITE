  ...
  return (
    <div data-screen-label="Track 201">
      ...
      <section style={{ background: "var(--bow-paper)", padding: "clamp(40px,5vw,72px) clamp(18px,4vw,40px) clamp(32px,4vw,56px)", borderBottom: "1px solid var(--border-rule)", overflow: "hidden", position: "relative" }}>
        <div className="bow-ghost bow-para-upbig" aria-hidden style={{ position: "absolute", right: -20, top: -60, fontFamily: "var(--font-display)", fontWeight: 900, fontSize: "clamp(220px,32vw,520px)", lineHeight: 0.7, color: "rgba(10,10,11,0.04)", pointerEvents: "none" }}>201</div>
      ...
      <section style={{ background: "#fff", padding: "clamp(48px,7vw,96px) clamp(18px,4vw,40px)", borderBottom: "1px solid var(--border-rule)" }}>
        <div className="bow-container">
          <SectionHeader kicker={`${track201Modules.length} Modules`} title="How the track is organized" style={{ marginBottom: 40 }} />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "clamp(16px,2vw,22px)", marginBottom: "clamp(20px,2.5vw,32px)" }}>
            {track201Modules.map((m: TrackModule) => (
              <div key={m.n} style={{ background: "#fff", border: "1px solid var(--border-rule)", borderTop: "4px solid var(--bow-orange)", padding: "22px 22px 26px", display: "flex", flexDirection: "column", gap: 8 }}>
